...
def tie_strengths(msg_array):
    ...
    for week in stats_array:
        ...
        for name in week:
            ...
            temp = tie - prev_strength
            temp = temp * 5
            deriv = ( temp + 1) / 2 
            values.append(tie)
            currdict[name] = [curr_stats[0], tie, deriv, curr_stats[6]]
        values.sort(reverse=True)
        results.append(currdict)
        culleddict = {}
        if len(values) > 12:
            for name in currdict:
                if currdict[name][1] > values[11]:
...
def getstringfromstats(name, stats_array):
    return '{ "tieStrength": ' + str(stats_array[1]) + ', "tieStrengthDerivative": ' + str(stats_array[2]) + ', "name": "' + name + '", "id": ' + str(stats_array[0]) + ', "week": "' + str(stats_array[3]) + '"}'
